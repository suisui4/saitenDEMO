...
def writeImg():
    ...
    # ファイルを開いてすべての行をリストで取得する
        ...
        return [list(map(str ,line.strip().split(','))) for line in lines[-n:]]
    ...
    for i,f in enumerate(files):
        ...
        #観点別点数を描画
        title_g, left_g, top_g, right_g, bottom_g = tail(img,2)[0]
        title_s, left_s, top_s, right_s, bottom_s = tail(img,2)[1]

        ginou, shikou = data_kanten[i]
        draw.text((int(int(right_g) - (int(right_g) - int(left_g))/1.5), int(int(bottom_g) - (int(bottom_g) - int(top_g))/1.5)), ginou, font=font, fill="red")
        draw.text((int(int(right_s) - (int(right_s) - int(left_s))/1.5), int(int(bottom_s) - (int(bottom_s) - int(top_s))/1.5)), shikou, font=font, fill="red")
