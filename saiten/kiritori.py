...
def name_lists(file,floor =10000, square=4000,gosa_tate=10):
    # 大問〇の△問目のリスト
    namelist=[]
    # 画像の読み込み、加工
    img = cv2.imread(file)
    ...
    # 左上から右下順になるようにソート
    rect_big = sorted(rect_big, key=lambda x: (x[0][0]))
    ...
    img = cv2.imread(file)
#     # 二段に分ける
    df.loc[df['end_x'] < img.shape[1]*8//15, 'label'] = 'left'
    df.loc[df['end_x'] > img.shape[1]*8//15, 'label'] = 'right'
    ...
    for i in range(len(df3)):
        x, t, x_end, t_end = df3.loc[i].to_list()
